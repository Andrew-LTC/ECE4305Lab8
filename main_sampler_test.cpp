...
//helper function to XADC Chasing LEDS
...
void XADC_Chasing_Leds(XadcCore *adc_p, GpoCore *led_p, GpiCore *sw_p, int ledAmt){
	int init, ledSpeed, ledSpeedOld;
	double adcSpeed;
	bool dir = true;
	int ledInitial = 1,ledShift = 1;

	init = sw_p->read(0);			//read sw0 for init

	while(init){
		led_p->write(1,0);			//keep rightmost led lit
		init = sw_p->read(0);		//poll sw0
	}
	while(!init){
		init = sw_p->read(0);
		adcSpeed = adc_p->read_adc_in(0);	//read channel 0
		ledSpeed = adcSpeed * 1000;	//equation for led speed output

		//if speed changes
		if(ledSpeed != ledSpeedOld){
		...
		//write 16 bits
		led_p->write(moveLed(ledInitial, ledShift, ledAmt, dir) & 0x0000ffff);
		ledShift++;
		sleep_ms(ledSpeed);
		led_p->write(0);
		sleep_ms(ledSpeed);

		//Reset shift amount when we reach end of LEDS and change direction
		if(ledShift == ledAmt){
			dir = !dir;
			ledShift = 0;
		}

		//save current speed for comparison later
		ledSpeedOld = ledSpeed;
	}
}
...
int main() {
   ...
   //timer_check(&led);
   while (1) {
	   XADC_Chasing_Leds(&adc,&led,&sw,16);
	   //adc_check(&adc, &led);
